module CampusSolutions
  class AdvisingResources < CachedProxy
    ...
    def fetch_academic_career(user_id)
      # get academic career by effective (most recent ) date
      hub_status = HubEdos::AcademicStatus.new(user_id: user_id).get
      return '' unless !hub_status[:feed]['student'].blank?

      academic_statuses = hub_status[:feed]['student']['academicStatuses']
      sorted_statuses = academic_statuses.sort_by { |status|
        status['studentCareer']['fromDate']
      }.reverse!

      sorted_statuses[0]['studentCareer']['academicCareer']['code']
    end
    ...
    def build_feed(response)
      ...
      if student_empl_id
        ...
        student_specific_link_settings = [
          ...
          { feed_key: :student_service_indicators, cs_link_key: 'UC_CX_SERVICE_IND_STDNT', cs_link_params: { :EMPLID => student_empl_id, :ACAD_CAREER => acad_career } },
          { feed_key: :student_webnow_documents, cs_link_key: 'UC_CX_WEBNOW_STUDENT_URI', cs_link_params: { :EMPLID => student_empl_id } },
        ]

        student_specific_link_settings.each do |setting|
          link = fetch_link(setting[:cs_link_key], setting[:cs_link_params])
          cs_links[setting[:feed_key]] = link unless link.blank?
        end
      end
